...
namespace Mat.Plugins
{
    public static class ImportZipFileHelper
    {
        ...
        public static void SubmitImportRecords(IOrganizationService serviceProxy, Guid userId, string base64String, ITracingService tracingService = null)
        {
            ...
            foreach (var recordType in recordTypes)
            {
                ...
                {
                    Guid importMapId = CreateMapping(serviceProxy, tracingService, recordType, timeStamp);
                    ...
                    foreach (var csvContentChunk in csvContentChunks)
                    {
                        ...
                        if (tracingService != null)
                            tracingService.Trace($"{nameof(SubmitImportRecords)} pre create import.");
                        Guid importId = serviceProxy.Create(import); // current import number incremented
                        if (tracingService != null)
                            tracingService.Trace($"{nameof(SubmitImportRecords)} post create immport.");
        ...
        new EntityReference(SystemUser.EntityLogicalName, userId);
                        ...
                        if (tracingService != null)
                            tracingService.Trace($"{nameof(SubmitImportRecords)} pre create import file.");
                        Guid importFileId = serviceProxy.Create(importFile);
                        if (tracingService != null)
                            tracingService.Trace($"{nameof(SubmitImportRecords)} post create immport file.");
                    }
                }
            }
        }

        private static Guid CreateMapping(IOrganizationService serviceProxy, ITracingService tracingService, RecordType recordType, string timeStamp)
        {
            ...
            if (tracingService != null)
                tracingService.Trace($"{nameof(SubmitImportRecords)} pre create import map.");
            Guid importMapId = serviceProxy.Create(importMap);
            if (tracingService != null)
                tracingService.Trace($"{nameof(SubmitImportRecords)} post create immport map.");
            switch (recordType)
            {
                case RecordType.PLZ:
        ...
        public static void StartSingleBulkDeleteJob(IOrganizationService serviceProxy, RecordType type, Guid userId, ITracingService tracingService = null)
        {
            ...
            if (tracingService != null)
                tracingService.Trace($"{nameof(StartSingleBulkDeleteJob)} pre create bulk delete job with user id {userId} and mail {GetCurrentUserEmail(serviceProxy, userId)}.");
            var bulkDeleteResponse =
                (BulkDeleteResponse)serviceProxy.Execute(bulkDeleteRequest);
            if (tracingService != null)
                tracingService.Trace($"{nameof(StartSingleBulkDeleteJob)} post create bulk delete job. Result response {bulkDeleteResponse}.");
        }

        public static void ScheduleBulkDeleteAsyncOperations(IOrganizationService serviceProxy, Guid userId, ITracingService tracingService = null)
        {
            ...
            if (tracingService != null)
                tracingService.Trace($"{nameof(ScheduleBulkDeleteAsyncOperations)} pre create bulk delete job with user id {userId} and mail {GetCurrentUserEmail(serviceProxy, userId)}");
            var bulkDeleteResponse =
                (BulkDeleteResponse)serviceProxy.Execute(bulkDeleteRequest);
            if (tracingService != null)
                tracingService.Trace($"{nameof(ScheduleBulkDeleteAsyncOperations)} post create bulk delete job. Result response {bulkDeleteResponse}.");
        }

        public static void BulkDeleteImportFiles(IOrganizationService serviceProxy, Guid userId, ITracingService tracingService = null)
        {
            string timeStamp = DateTime.Now.ToString("yyyyMMddHHmmss");
            var deleteFilter = new FilterExpression();
            deleteFilter.AddCondition(new ConditionExpression(AsyncOperation.Fields.OperationType, ConditionOperator.Equal, AsyncOperation_OperationType.Import));
            ...
            if (tracingService != null)
                tracingService.Trace($"{nameof(BulkDeleteImportFiles)} pre create bulk delete job with user id {userId} and mail {GetCurrentUserEmail(serviceProxy, userId)}");
            var bulkDeleteResponse =
                (BulkDeleteResponse)serviceProxy.Execute(bulkDeleteRequest);
            if (tracingService != null)
                tracingService.Trace($"{nameof(BulkDeleteImportFiles)} post create bulk delete job. Result response {bulkDeleteResponse}.");
        }

        public static void StartSubmitedImportJobs(IOrganizationService serviceProxy, RecordType type, ITracingService tracingService = null, string name = null)
        {
            List<Entity> jobs;
            if (name == null)
            {
                jobs = RetrieveSubmitedImortJobs(serviceProxy, tracingService);
            }
            else
            {
                jobs = RetrieveSubmitedImortJobsByJobIndex(serviceProxy, tracingService, name);
            }
            if (tracingService != null)
                tracingService.Trace($"{nameof(StartSubmitedImportJobs)} post retrieve import jobs. Result count: {jobs.Count}.");
            foreach (var job in jobs)
            {
                ...
                switch (type)
                {
                    case RecordType.PLZ:
                        ...
                        {
                            if (tracingService != null)
                                tracingService.Trace($"{nameof(StartSingleImportJob)} pre StartSingleImportJob. Type {type} and name {importName}.");
                            StartSingleImportJob(serviceProxy, job.Id, tracingService);
                            if (tracingService != null)
                                tracingService.Trace($"{nameof(StartSingleImportJob)} post StartSingleImportJob. Type {type}.");
                        }
                        break;
                    case RecordType.STR:
        ...
        private static List<Entity> RetrieveSubmitedImortJobs(IOrganizationService serviceProxy, ITracingService tracingService)
        {
            ...
            if (tracingService != null)
                tracingService.Trace($"{nameof(RetrieveSubmitedImortJobs)} pre retrieve import jobs.");
            var jobs = serviceProxy.RetrieveMultiple(query).Entities.ToList();
            return jobs;
        }

        private static List<Entity> RetrieveSubmitedImortJobsByJobIndex(IOrganizationService serviceProxy, ITracingService tracingService, string jobName)
        {
            ...
            if (jobName.Length > 43)
            {
                ...
                if (tracingService != null)
                    tracingService.Trace($"{nameof(RetrieveSubmitedImortJobsByJobIndex)} pre retrieve import jobs. Job index {jobIndex}");
        ...
        private static void StartSingleImportJob(IOrganizationService serviceProxy, Guid importId, ITracingService tracingService = null)
        {
